...
const useStyles = makeStyles(() => ({
  ...
  ctaButton: {
    marginTop: '2rem',
    animation: '$fadeInSlideUp 2s ease-in-out 0.5s', // Delayed button entry
  },
...
const Homepage = () => {
  ...
  const handleCardClick = (feature) => {
    // ... (your card click handler logic) ...
      if (feature === 'Emission Quantification') {
        // Assuming you're using react-router-dom for navigation
        window.location.href = '/EmissionSummary'; 
      }
      if (feature === 'Carbon Neutrality Pathways'){
        window.location.href = '/Mainneutrality';
      }
      if (feature === 'Coal Mine Summary'){
        window.location.href = '/MineSummary';
      }
      if (feature === 'Mines Locations'){
        window.location.href = '/MineLocation';
      }
  ...
  <Grid container spacing={3} style={{ marginTop: '2rem' }}>
    ...
    <Grid item xs={12} sm={6}>
      <Box className={classes.emptyBox}> 
        {/* This is the empty box for future content */}
      </Box>
    </Grid>
  </Grid>
</Box>
